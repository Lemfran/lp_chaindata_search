...
def fetch_mints_data(skip, timestamp_gte):
    """获取mints数据"""
    headers = {
    ...
    while retry_count < max_retries:
        try:
            ...
            if 'data' not in res or 'mints' not in res['data']:
                print("API返回异常：", res)
                return None
            
            return res['data']['mints']
...
def append_to_csv(mints_data, total_records_processed):
    """将数据写入到当前批次的CSV文件"""
    # 检查是否有数据
    if not mints_data:
        print("没有数据可保存")
    ...
    with open(current_csv_file, mode=mode, newline='', encoding='utf-8') as csv_file:
        ...
        for mint in mints_data:
            writer.writerow({
                ...
                'transaction_blockNumber': mint['transaction']['blockNumber'],
    ...
    print(f"数据已成功写入到 {current_csv_file}，新增 {len(mints_data)} 条记录，文件总记录数: {total_records_in_file}")


def main():
    """主函数，实现分页查询并按批次保存数据到不同的CSV文件"""
    all_mints = []  # 用于跟踪所有数据以便记录最后时间戳
    skip = INITIAL_SKIP
    ...
    try:
        while True:
            ...
            mints = fetch_mints_data(skip, timestamp_gte)
            ...
            # 更新skip值，准备获取下一页数据
            # 使用原始的mints长度来更新skip，因为API是按原始数量分页的
            skip += len(mints)
        ...
        if all_mints:
            ...
            print(f"已保存 {len(all_mints)} 条新记录")
            return
        else:
            print("没有获取到任何新数据")
            return
